...
int main() {
    cin >> n1 >> n2;
    if (strlen(n1) < strlen(n2) || strlen(n1) == strlen(n2) && strcmp(n1, n2) < 0) { // 判断结果是否是一个负数
        // strcmp()函数是比较两个字符串(字符数组)的关系,n1 == n2返回0,n1 < n2返回负整数,n1 > n2返回正整数
        strcpy(n, n1);
        strcpy(n1, n2);
        strcpy(n2, n);
        // strcpy()函数是把一个字符串(字符数组)赋值整体给另一个字符串(字符数组),strcpy就是string copy的缩写
        cout << "-"; // 如果是一个负数就先输出负号
    }
    ...
    lenc = i;
    while (c[lenc] == 0 && lenc > 1) lenc--; // 判断当前位数字为零并且非最高位,其实就是删除前导0
    // 输出,注意:如果我们这么处理就要倒序输出,因为我们转换时倒序存储
    for (i = int(lenc); i >= 1; i--) cout << c[i];
    cout << endl;
    return 0;
}
