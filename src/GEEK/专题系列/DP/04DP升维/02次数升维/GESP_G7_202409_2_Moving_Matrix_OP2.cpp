...
const int NEG = -1e9;
vector<string> g(505);
vector<vector<int>> dp(505, vector<int>(305, NEG));
...
int main() {
    ...
    while (t--) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                ...
                for (int k = K; k >= 0; k--) {
                    // 从上方或左方转移（不改字符，k 不变）
                    int from_k = NEG;
                    if (i > 0) from_k = max(from_k, dp[j][k]);
                    if (j > 0) from_k = max(from_k, dp[j - 1][k]);
                    // 从上方或左方转移（修改字符，k-1 变 k）
                    int from_km1 = NEG;
                    if (k >= 1 && i > 0) from_km1 = max(from_km1, dp[j][k - 1]);
                    if (k >= 1 && j > 0) from_km1 = max(from_km1, dp[j - 1][k - 1]);
        ...
        // 答案：取终点所有 k 的最大值
        int ans = 0;
        for (int k = 0; k <= K; k++) ans = max(ans, dp[m - 1][k]);
        cout << ans << endl;
    }
    return 0;
}